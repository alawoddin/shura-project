...
@section('admin')
    ...
    <div class="container-fluid my-0">
        <div class="py-3">
            <h4 class="fs-18 fw-semibold m-0">ویرایش نماینده</h4>
        </div>
        ...
        <div class="row">
            <div class="col-xl-12">
                <div class="card">
                    <div class="card-body">
                        <form id="myForm" action="{{ route('update.representative') }}" method="POST" class="row g-3">
                            @csrf
                                ...
                                <select name="ethnic_branch_id" class="form-control">
                                    ...
                                    @foreach ($ethnicBranches as $branch)
                                        <option value="{{ $branch->id }}"
                                            {{ old('ethnic_branch_id', $representative->ethnic_branch_id) == $branch->id ? 'selected' : '' }}>
                                            {{ $branch->name }}
                                        </option>
                                    @endforeach
                                </select>
